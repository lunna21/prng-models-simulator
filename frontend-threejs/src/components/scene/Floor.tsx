...
function createWallGradientTexture(): THREE.CanvasTexture {
  ...
  const stars: [number, number, number][] = [
    [40, 55, 3], [110, 28, 2.5], [175, 75, 2], [240, 38, 3], [310, 90, 2],
    [380, 22, 2.5], [450, 65, 2], [60, 130, 2], [200, 115, 3], [340, 140, 2],
    [480, 110, 2.5], [90, 185, 2], [270, 170, 2.5], [420, 195, 2],
  ];
  ...
  return (
    <group position={position}>
      <mesh position={[0, 0.05, 0]} receiveShadow castShadow>
        <cylinderGeometry args={[0.18, 0.20, 0.10, 22]} />
        <meshPhysicalMaterial color="#DCC882" roughness={0.36} metalness={0} clearcoat={0.55} />
      </mesh>
      <mesh position={[0, 0.54, 0]} castShadow>
        <cylinderGeometry args={[0.036, 0.040, 0.90, 14]} />
        <meshPhysicalMaterial color="#EAD898" roughness={0.30} metalness={0} clearcoat={0.65} />
      </mesh>
      <mesh position={[0, 1.01, 0]} castShadow>
        <sphereGeometry args={[0.082, 20, 20]} />
        <meshPhysicalMaterial color="#EAD898" roughness={0.25} metalness={0} clearcoat={0.75} />
      </mesh>
    </group>
  );
}
...
function QueueRope({
  ...
  return (
    <mesh geometry={geo} castShadow>
      <meshPhysicalMaterial color="#7BBFDB" roughness={0.42} metalness={0} clearcoat={0.45} />
    </mesh>
  );
}
  ...
  return (
    <>
      {([-1.2, 0, 1.2] as number[]).map((x) => (
        <mesh key={x} rotation={[-Math.PI / 2, 0, 0]} position={[x, 0.025, z]} receiveShadow>
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0.048, -1.35]} castShadow receiveShadow>
        <boxGeometry args={[14, 0.096, 0.30]} />
        <meshPhysicalMaterial color="#C0B0D4" roughness={0.40} metalness={0} clearcoat={0.40} />
      </mesh>
      {/* Gold nosing strip */}
      <mesh position={[0, 0.100, -1.22]} castShadow>
        <boxGeometry args={[14, 0.022, 0.055]} />
        <meshPhysicalMaterial color="#E0C87A" roughness={0.26} metalness={0.05} clearcoat={0.72} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 0.26, 0]} castShadow>
        <cylinderGeometry args={[0.24, 0.20, 0.52, 22]} />
        <meshPhysicalMaterial color="#F2EEE8" roughness={0.38} clearcoat={0.55} />
      </mesh>
      {/* Pot rim */}
      <mesh position={[0, 0.54, 0]} castShadow>
        <cylinderGeometry args={[0.26, 0.26, 0.07, 22]} />
        <meshPhysicalMaterial color="#E8E2DC" roughness={0.38} clearcoat={0.4} />
      </mesh>
      {/* Trunk */}
      <mesh position={[0, 0.92, 0]} castShadow>
        <cylinderGeometry args={[0.072, 0.10, 0.80, 12]} />
        <meshPhysicalMaterial color="#9C7C50" roughness={0.68} />
      </mesh>
      {/* Main canopy */}
      <mesh position={[0, 1.54, 0]} castShadow>
        <sphereGeometry args={[0.56, 22, 22]} />
        <meshPhysicalMaterial color="#48B870" roughness={0.42} clearcoat={0.38} />
      </mesh>
      {/* Secondary puff clusters */}
      <mesh position={[0.24, 1.44, 0.14]} castShadow>
        <sphereGeometry args={[0.32, 16, 16]} />
        <meshPhysicalMaterial color="#5ACC80" roughness={0.42} clearcoat={0.32} />
      </mesh>
      <mesh position={[-0.20, 1.40, -0.10]} castShadow>
        <sphereGeometry args={[0.26, 14, 14]} />
        <meshPhysicalMaterial color="#42B268" roughness={0.44} clearcoat={0.30} />
      </mesh>
    </group>
  );
}
...
function WallFrames() {
  const frames = [
    { x: -4.45, y: 1.25, z: -0.03, innerColor: '#C8DEBA', accent: '#8FD1A7', w: 1.36, h: 1.18, rot: -0.11 },
    { x: -1.85, y: 0.88, z: 0.02, innerColor: '#BED4DC', accent: '#7BBFDB', w: 1.2, h: 0.96, rot: 0.08 },
    { x: 1.85, y: 1.08, z: -0.01, innerColor: '#D4C8B8', accent: '#E8C87A', w: 1.22, h: 0.95, rot: -0.07 },
    { x: 4.55, y: 0.82, z: 0.03, innerColor: '#C8C4DC', accent: '#AFA2E8', w: 1.34, h: 1.02, rot: 0.12 },
  ];
      ...
      {frames.map(({ x, y, z, innerColor, accent, w, h, rot }, i) => (
        <group key={x} position={[x, y, z]} rotation={[0, 0, rot]}>
          ...
          <mesh castShadow>
            ...
            <meshPhysicalMaterial color="#E8DACA" roughness={0.36} metalness={0} clearcoat={0.45} />
          </mesh>

          {/* Inner bezel */}
          <mesh position={[0, 0, 0.04]} castShadow>
            <boxGeometry args={[w + 0.08, h + 0.08, 0.05]} />
            <meshPhysicalMaterial color="#F3E8D8" roughness={0.3} metalness={0} clearcoat={0.52} />
          </mesh>

          {/* Art panel */}
          <mesh position={[0, 0, 0.075]}>
            <planeGeometry args={[w, h]} />
            <meshPhysicalMaterial color={innerColor} roughness={0.28} metalness={0} clearcoat={0.6} />
          </mesh>
          ...
          {i === 0 && (
            <group position={[0, -0.04, 0.09]}>
              <mesh position={[-0.09, 0.05, 0]}>
                ...
                <meshPhysicalMaterial color="#91DDA8" roughness={0.32} clearcoat={0.7} />
              </mesh>
              <mesh position={[0.09, 0.05, 0]}>
                <sphereGeometry args={[0.08, 14, 14]} />
                <meshPhysicalMaterial color="#91DDA8" roughness={0.32} clearcoat={0.7} />
          ...
          {i === 2 && (
            <mesh position={[0, -0.02, 0.09]} rotation={[0.1, 0.15, 0.4]}>
              ...
              <meshPhysicalMaterial color="#F7E3A1" roughness={0.28} clearcoat={0.6} />
            </mesh>
          )}
          ...
          {i === 3 && (
            <group position={[0, -0.02, 0.09]}>
              <mesh rotation={[0, 0, 0.2]}>
                ...
                <meshPhysicalMaterial color="#C6B9F2" roughness={0.32} clearcoat={0.65} />
              </mesh>
              <mesh position={[0, 0, 0.02]} rotation={[0, 0, -0.25]}>
                <tetrahedronGeometry args={[0.05, 0]} />
                <meshPhysicalMaterial color="#FFFFFF" roughness={0.3} clearcoat={0.52} />
              </mesh>
            </group>
          )}
          ...
          <mesh position={[0, -(h * 0.5) - 0.08, 0.02]} castShadow>
            ...
            <meshPhysicalMaterial color="#EAD7BE" roughness={0.34} clearcoat={0.4} />
          </mesh>
        </group>
      ))}
    </group>
  ...
  return (
    <>
      <mesh position={[0, 0.12, -7.72]} castShadow>
        ...
        <meshPhysicalMaterial color="#D4C8DC" roughness={0.42} clearcoat={0.38} />
      </mesh>
      <mesh position={[0, 0.25, -7.70]} castShadow>
        <boxGeometry args={[width, 0.025, 0.05]} />
        <meshPhysicalMaterial color="#E8DFF0" roughness={0.28} clearcoat={0.60} />
      </mesh>
    </>
  );
}
...
export function Floor({ width = 20, depth = 16 }: FloorProps) {
  ...
  return (
    <group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.01, -3.8]} receiveShadow>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.012, 3.2]} receiveShadow>
        ...
        <meshPhysicalMaterial map={carpetTexture} roughness={0.52} metalness={0} clearcoat={0.12} />
      </mesh>
      {/* Carpet edge bars */}
      ...
      {/* ── QUEUE POSTS & ROPES ──────────────────────────────── */}
      {postsLeft.map((pos, i)  => <QueuePost key={`L${i}`} position={pos} />)}
      {postsRight.map((pos, i) => <QueuePost key={`R${i}`} position={pos} />)}
      ...
      {/* ── SIDE WALLS ───────────────────────────────────────── */}
      <mesh position={[-(width / 2) + 0.06, 3.8, 0]} rotation={[0, Math.PI / 2, 0]}>
